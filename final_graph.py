...
number_episodes = [100,10**5,100000,3*(10**6),15*(10**6)] #for each maze, different number of learning steps
...
for i in range(number_simulations-1):
	print(i+1)
	j = 0
	for action_selection in alg_list:
		r1, r2, total_reward = simulation_multiple_episodes(number_episodes[maze_type],action_selection,max_it,N_pos,N_actions,maze_parameters[maze_type],interval_reward_storage[maze_type],maze_type)
		total_reward_list[j] += total_reward
		j += 1

j = 0
for action_selection in alg_list:
	total_reward_list[j] /= number_simulations
	j+=1
